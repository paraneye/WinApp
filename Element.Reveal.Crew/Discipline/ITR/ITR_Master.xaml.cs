...
namespace Element.Reveal.Crew.Discipline.ITR
{
    ...
    public sealed partial class ITR_Master : WinAppLibrary.Controls.LayoutAwarePage
    {
        ...
        private async void CheckDef()
        {
            ...
            this.Frame.Navigate(typeof(FillOutSubmitITR), "1");
            Login.MasterPage.DoBeforeBack -= MasterPage_DoBeforeBack;    
        }
        ...
        private async void SaveDoc()
        {
            ...
            if (Doc.Status != EStatusType.ReadyToSubmit)
            {
                ...
                if (blSaved)
                {
                    ...
                    if (saveresult)
                        MessageHide("Save Completed", "Saved");
                    else
                        MessageHide("Save Failed", "Save Fail");
            ...
            switch (Login.LoginMode)
            {
                case WinAppLibrary.UI.LogMode.OnMode:
                    ...
                    if (await Doc.Submit())
                    {
                        ...
                        if (res)
                        {
                            MessageHide("Submit completed!", "Submit Complete");
                            this.Frame.Navigate(typeof(FillOutSubmitITR), 1);
                        }
                        else
                            MessageHide("Submit Failed!", "Submit Failed");
