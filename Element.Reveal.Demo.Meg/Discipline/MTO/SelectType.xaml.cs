    ...
    public sealed partial class SelectType : WinAppLibrary.Controls.LayoutAwarePage
    {
        ...
        private async void LoadType()
        {
            ...
            try
            {
                if (Login.LoginMode == WinAppLibrary.UI.LogMode.OnMode)
                {
                    lvTaskType.ItemsSource = await (new Lib.ServiceModel.CommonModel()).GetLookupByLookupType_Combo(WinAppLibrary.Utilities.LOOKUPTYPE.ComponentTaskType, TaskTypeName(Lib.CommonDataSource.selectedMaterialCategory));
                }
                else
                {
