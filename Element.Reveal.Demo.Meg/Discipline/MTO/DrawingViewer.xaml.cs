...
namespace Element.Reveal.Meg.Discipline.MTO
{
    ...
    public sealed partial class DrawingViewer : WinAppLibrary.Controls.LayoutAwarePage
    {
        ...
        RevealProjectSvc.MTOPageTotal mtolist = new RevealProjectSvc.MTOPageTotal();
        ...
        private async void LoadMtoList()
        {
            try 
            {
                mtolist = await(new Lib.ServiceModel.ProjectModel()).GetMaterialTakeOff(Lib.CWPDataSource.selectedCWP, Lib.CommonDataSource.selectedDrawing,
                    Lib.CommonDataSource.selectedMaterialCategory, _projectid, _moduleid);
        ...
        private async void btnDelete_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    RevealProjectSvc.MTOPageTotal deletemto = new RevealProjectSvc.MTOPageTotal();
                    foreach (RevealProjectSvc.MTODTO mto in mtolist.mto)
                    {
                        //todo equals 항목 확인
                        if (mto.ProgressID.Equals(((RevealProjectSvc.MTODTO)lvMtoList.SelectedItem).ProgressID))
                        {
                            mto.DTOStatus = (int)WinAppLibrary.Utilities.RowStatus.Delete;
                            deletemto.mto = new List<RevealProjectSvc.MTODTO>();
