...
namespace Element.Reveal.Meg.Discipline.ITR
{
    ...
    public sealed partial class FillOutSubmitITR : WinAppLibrary.Controls.LayoutAwarePage
    {
        ...
        protected override void LoadState(Object navigationParameter, Dictionary<String, Object> pageState)
        {
            ...
            _loginid = string.IsNullOrEmpty(WinAppLibrary.Utilities.Helper.LoginID) ? Login.UserAccount.PersonnelID.ToString() : WinAppLibrary.Utilities.Helper.LoginID;
        ...
        {
            BaseFolder = BaseFolder == null ? Lib.ContentPath.OffModeUserFolder : BaseFolder;
        ...
        private async void BindList()
        {
            ...
            try
            {
                ...
                if (_ofiles != null && _ofiles.Count > 0)
                {
        ...
        private async void bindListfromService()
        {
            ...
            try
            {
                ...
                if (_oservices == null || _oservices.Count < 0)
                {
                    WinAppLibrary.Utilities.Helper.SimpleMessage("Not Found Data", "Not Found Data!");
                    return;
                }
                ...
                int fiwpid = -1;
                List<FiwpqaqcDTO> itrs = new List<FiwpqaqcDTO>();
                foreach (QaqcformDTO item in _oservices)
                {
                    if (fiwpid != item.FIWPID)
                    {
                        itrs = await project.GetITRListByFiwp(item.FIWPID);
                        fiwpid = item.FIWPID;
                    }
                    ...
                    foreach (FiwpqaqcDTO ditem in itrs)
                    {
                        ...
                        {
                            data.QAQCFormTemplateName = ditem.QAQCFormTemplateName + "_" + item.KeyValue;
                            data.ExceptionMessage = ditem.QAQCFormCode;
                            break;
                        }
                    }
                ...
                if (gvDocument.SelectedItems.Count > 0)
                {
                    ...
                    switch (departmentId)
                    {
                        case WinAppLibrary.Utilities.Department.Foreman:
                            filename = dto.QAQCFormTemplateID.ToString() + "_" + dto.ExceptionMessage + ".xml";
                        ...
                        case WinAppLibrary.Utilities.Department.QualityManagement:
                            foreach (QaqcformDTO item in _oservices)
                            {
                                ...
                                {
                                    filename = item.QAQCFormID.ToString() + "_" + dto.ExceptionMessage + ".xml";
        ...
        private async void RemoveITR()
        {
            ...
            try
            {
                if (gvDocument.SelectedItems.Count > 0)
                {
                    QaqcformtemplateDTO target = (QaqcformtemplateDTO)gvDocument.SelectedItems.ToList().FirstOrDefault();

                    if (_ofiles == null || _ofiles.Count < 1)
                    {
                        return;
                    }

                    forRemove = _ofiles.Where(c => c.QAQCFormTemplateID == target.QAQCFormTemplateID).ToList();  // 삭제
                    forKeep = _ofiles.Where(c => c.QAQCFormTemplateID != target.QAQCFormTemplateID).ToList();  // 유지

                    foreach (QaqcformtemplateDTO data in _ofiles)
                    {
                        if (data.QAQCFormTemplateID == target.QAQCFormTemplateID)
                        {
                            QaqcformDTO dto = new QaqcformDTO();
                            filename = data.QAQCFormTemplateID.ToString() + "_" + data.ExceptionMessage + ".xml";
                            ...
                            foreach (QaqcformdetailDTO item in dto.QaqcfromDetails)
                            {
                                item.DTOStatus = (int)WinAppLibrary.Utilities.RowStatus.Delete;  // QaqcformdetailDTO Delete mark
                            }
                    ...
                    switch (Login.LoginMode)
                    {
                        ...
                        case WinAppLibrary.UI.LogMode.OnMode:
                            ...
                            {
                                filename = item.QAQCFormTemplateID.ToString() + "_" + item.ExceptionMessage + ".xml";
