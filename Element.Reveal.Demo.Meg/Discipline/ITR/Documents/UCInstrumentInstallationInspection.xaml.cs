...
namespace Element.Reveal.Meg.Discipline.ITR
{
    public sealed partial class UCInstrumentInstallationInspection : UserControl, IItrDoc
    {
        ...
        public void DoAfter(RevealProjectSvc.QaqcformDTO _dto)
        {
            ...
            txtIn.Text = (!string.IsNullOrEmpty(_dto.FIWPName)) ? _dto.FIWPName : "";
            txtSno.Text = (!string.IsNullOrEmpty(_dto.SystemNumber)) ? _dto.SystemNumber : "";
            txtSna.Text = (!string.IsNullOrEmpty(_dto.SystemName)) ? _dto.SystemName : "";
            txtInstrumentTagNo.Text = (!string.IsNullOrEmpty(_dto.TagNumber)) ? _dto.TagNumber : "";

            List<DrawingDTO> drawingDto = _dto.QaqcrefDrawing;
            if (drawingDto != null)
                lvDataSheet.ItemsSource = drawingDto;
        }
