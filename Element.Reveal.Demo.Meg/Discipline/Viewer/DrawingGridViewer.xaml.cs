                    ...
                    if (writableImage != null)
                    {
                        using (Stream stream = await (new WinAppLibrary.Utilities.Helper()).GetJpegStreamFromWriteableBitmap(writableImage))
                        {
                            if (DrawingEditor.EditMode == DrawingEditMode.RFI)
                            {
                                ...
                                {
                                    await (new WinAppLibrary.Utilities.WebDavClient_Old(this.DrawingEditor.FileServer + this.DrawingEditor.FileName, Login.UserAccount.WDUser, Login.UserAccount.WDPassword, "")).
                                        UploadImageContent(DrawingEditor.FileServer + DrawingEditor.FileName, webdav);
                        ...
                        WinAppLibrary.Utilities.Helper.SimpleMessage("There was an error to read drawing board. Please contact administrator", "Caution!");
                }
                catch (Exception e)
                {
                    (new WinAppLibrary.Utilities.Helper()).ExceptionHandler(e, "DawingGridView SaveDrawing", 
                        "We failed to save the modified drawing. Please contact administrator.", "Caution!");
