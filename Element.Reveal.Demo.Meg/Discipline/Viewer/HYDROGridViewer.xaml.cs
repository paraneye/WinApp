...
namespace Element.Reveal.Meg.Discipline.Viewer
{
    ...
    /// </summary>
    public sealed partial class HYDROGridViewer : WinAppLibrary.Controls.LayoutAwarePage
    {
        const double ANIMATION_SPEED = WinAppLibrary.Utilities.AnimationHelper.ANIMATION_TIMEs;
        Lib.UI.HydroOption _hydrooption = new Lib.UI.HydroOption();
        Lib.IWPDataSource _hydrosource = new Lib.IWPDataSource();
        ...
        private void _hydrooption_ViewClicked(object sender, object e)
        {
            try
            {
                Loaddrawing();
            ...
            {
                (new WinAppLibrary.Utilities.Helper()).ExceptionHandler(ex, "_hydrooption_ViewClicked", "There was an error load IWP drawing. Pleae contact administrator", "Error!");
            }
        }
        ...
        private async void LoadDocument()
        {
            try
            {
                var item = _hydrooption.SelectredHYDRO as RevealCommonSvc.ComboBoxDTO;
                if (item != null)
                {
                ...
                _documentdto = await _hydrosource.GetItrDocumentForFIWPByFIWPID(WinAppLibrary.Utilities.DocEstablishedForApp.ITR, item.DataID, _projectid, _moduleid);
        ...
        private async void LoadOption()
        {
            ...
            _hydrooption.ViewClicked += _hydrooption_ViewClicked;
            _hydrooption.ActiveOpacity = 1;
            this.StretchingPanel.AddPanel(_hydrooption);
            this.StretchingPanel.NavigateTo(_hydrooption.Title);
        ...
        private async Task<bool> LoadOptionOnMode()
        {
            ...
            try
            {
                await _hydrosource.GetSIWPByProjectScheduleOnMode(0);
        ...
        private async void Loaddrawing()
        {
                ...
                if (Login.LoginMode == WinAppLibrary.UI.LogMode.OnMode)
                {
                    ...
                    if (login)
                    {
                        ...
                        if (item != null)
                        {
                            ...
                            await _hydrosource.GetIwpDrawingOnMode(item.DataID, _projectid, _moduleid, Lib.MainMenuList.HydroViewer);
                            source = _hydrosource.GetGroupedDocument("System Installation Work Package");
            ...
            {
                (new WinAppLibrary.Utilities.Helper()).ExceptionHandler(e, "HYDROGridViewer Loaddrawing", "There was an error load drawing. Pleae contact administrator", "Error!");
            }
