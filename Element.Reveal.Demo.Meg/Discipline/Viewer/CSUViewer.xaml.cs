...
namespace Element.Reveal.Meg.Discipline.Viewer
{
    ...
    public sealed partial class CSUViewer : WinAppLibrary.Controls.LayoutAwarePage
    {
        ...
        private async void LoadDocument()
        {
            try
            {
                var item = _csuoption.SelectredCSU as RevealProjectSvc.FiwpDTO;
                if (item != null)
                {
                ...
                _documentdto = await _csusource.GetItrDocumentForFIWPByFIWPID(WinAppLibrary.Utilities.DocEstablishedForApp.ITR, item.FiwpID, _projectid, _moduleid);
            }
            catch
            {
            }
                ...
                if (Login.LoginMode == WinAppLibrary.UI.LogMode.OnMode)
                {
                    ...
                    if (login)
                    {
                        ...
                        if (item != null)
                        {
                            ...
                            await _csusource.GetIwpDrawingOnMode(item.FiwpID, _projectid, _moduleid, Lib.MainMenuList.CSUViewer);
