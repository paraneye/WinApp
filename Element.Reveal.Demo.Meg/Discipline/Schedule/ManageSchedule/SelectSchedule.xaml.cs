    ...
    public sealed partial class SelectSchedule : WinAppLibrary.Controls.LayoutAwarePage
    {
        ...
        private void gvSchedule_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (e.AddedItems.Count > 0)
            {
                ...
                Lib.ScheduleDataSource.selectedScheduleStartDate = string.IsNullOrEmpty(scheduleDate[0]) ? "" : Convert.ToDateTime(scheduleDate[0]).ToString("MMM dd, yyyy");
                Lib.ScheduleDataSource.selectedScheduleEndDate = string.IsNullOrEmpty(scheduleDate[1]) ? "" : Convert.ToDateTime(scheduleDate[1]).ToString("MMM dd, yyyy");
