...
namespace Element.Reveal.Meg.Discipline.Schedule.AssembleIWP 
{
    ...
    public sealed partial class SelectSchedule : WinAppLibrary.Controls.LayoutAwarePage
    {
        ...
        private int _projectid, _moduleid, _packagetypeLuid;
        ...
        private async void gvSchedule_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (e.AddedItems.Count > 0)
            {
                ...
                {
                    List<RevealProjectSvc.FiwpDTO> iwps = Lib.IWPDataSource.iwplist.Where(x => x.FiwpID == Lib.IWPDataSource.selectedSIWP).ToList();
                    RevealProjectSvc.ProjectscheduleDTO psdto =  _schedule.Schedule.Where(x => x.ProjectScheduleID == Lib.ScheduleDataSource.selectedSchedule).SingleOrDefault();
                    foreach (RevealProjectSvc.FiwpDTO dto in iwps)
                    {
                        dto.DTOStatus = (int)WinAppLibrary.Utilities.RowStatus.Update;
                        dto.ModuleID = psdto.ModuleID;
        ...
        private async void LoadSchedule()
        {
            ...
            try
            {
                if (Login.LoginMode == WinAppLibrary.UI.LogMode.OnMode)
                {
                    ...
                    {
                        await _schedule.GetProjectScheduleByCwpProjectIDWithWBSOnMode(0, _projectid, _moduleid);
                    }
