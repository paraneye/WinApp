...
namespace Element.Reveal.Meg.Discipline.Schedule.BuildSIWP 
{
    ...
    /// </summary>
    public sealed partial class SelectSIWP : WinAppLibrary.Controls.LayoutAwarePage
    {
       ...
       // Lib.UI.ScheduleDetail _scheduleDetail = new Lib.UI.ScheduleDetail();
        ...
        {
            if (Lib.IWPDataSource.selectedSIWP != null && Lib.IWPDataSource.selectedSIWP > 0)
            {
                await _siwp.GetSiwpByIDOnMode(Lib.IWPDataSource.selectedSIWP);
                List<RevealProjectSvc.FiwpDTO> siwps = _siwp.GetSiwpByID();

                if (siwps.Count > 0)
                {
                    RevealProjectSvc.FiwpDTO siwp = siwps[0];
                    ...
                    {
                        if (dto.DataID == siwp.DepartStructureID)
                        {
                    ...
                    foreach (RevealCommonSvc.LookupDTO dto in cbUTestType.Items)
                    {
            ...
            {
                if (_siwp.GetSiwpByProjectSchedule().Count() > 0)
                {
                    WinAppLibrary.Utilities.Helper.SimpleMessage("Alert!", "Please Select SIWP.");
                }
                else
                {
                    WinAppLibrary.Utilities.Helper.SimpleMessage("Alert!", "There is no SIWP data. Please Create SIWP.");
                }
            }
        }
        ...
        private void _iwpDetail_btnNewIWPClicked(object sender, object e)
        {
            ...
            foreach (RevealCommonSvc.LookupDTO dto in cbNTestType.Items)
            {
        ...
        public async void GetForemanList(int generalForemanId)
        {
            ...
            await _siwp.GetForemanByGeneralForemanOnMode(generalForemanId, _projectid, _moduleid);
        ...
        private async void btnNSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            newFiwp.TestTypeLUID = (cbNTestType.SelectedValue as RevealCommonSvc.LookupDTO).LookupID;

            newFiwps.Add(newFiwp);
           
            var result = await _siwp.SaveFIWP(newFiwps);
        ...
        private async void btnUSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            List<RevealProjectSvc.FiwpDTO> iwps = _siwp.GetSiwpByID();
            ...
            if (iwps.Count > 0)
            {
                ...
                iwps[0].TestTypeLUID = (cbUTestType.SelectedValue as RevealCommonSvc.LookupDTO).LookupID;
            ...
            var result = await _siwp.SaveFIWP(iwps);
