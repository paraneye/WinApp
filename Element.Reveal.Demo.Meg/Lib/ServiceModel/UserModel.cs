...
namespace Element.Reveal.Meg.Lib.ServiceModel
{
    public class UserModel
    {
        ...
        public async Task<RevealUserSvc.LoginaccountDTO> GetLoginaccountByTempOwnerPesonnelID(int personnelId)
        {
            RevealUserSvc.UserServiceClient user = ServiceHelper.GetServiceClient<RevealUserSvc.UserServiceClient>(ServiceHelper.UserService);
            var retValue = await user.GetLoginaccountByTempOwnerPesonnelIDAsync(Helper.DBInstance, personnelId);
            await user.CloseAsync();
            return retValue;
        }
