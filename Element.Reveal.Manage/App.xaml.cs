    ...
    sealed partial class App : Application
    {
        ...
        protected async override void OnLaunched(LaunchActivatedEventArgs args)
        {
            ...
            if (rootFrame == null)
            {
                ...
                rootFrame.HomeClicked += (s, e) =>
                {
                    if (!rootFrame.ContentFrame.Navigate(typeof(MainMenu), Login.UserAccount.PersonnelID))
                    {
                        throw new Exception("Failed to create initial page");
                    }
                };

                rootFrame.OptionSaved += (s, e) =>
                {
                    Login.UserAccount.CurProjectID = rootFrame.CurProjectId;
                    Login.UserAccount.CurModuleID = rootFrame.CurModuleId;

                    if (!rootFrame.ContentFrame.Navigate(typeof(MainMenu), true))
                    {
                        throw new Exception("Failed to create initial page");
                    }
                };
            ...
            if (rootFrame.ContentFrame.Content == null)
            {
                ...
                // configuring the new page by passing required information as a navigation
                // parameter
                if (!rootFrame.ContentFrame.Navigate(typeof(Login), args.Arguments))
                {
                    throw new Exception("Failed to create initial page");
                }
            }
