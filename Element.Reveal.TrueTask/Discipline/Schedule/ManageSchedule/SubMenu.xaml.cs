...
namespace Element.Reveal.TrueTask.Discipline.Schedule.ManageSchedule
{
    ...
    {
        List<DataLibrary.ExtSchedulerDTO> source = new List<DataLibrary.ExtSchedulerDTO>();
        ...
        protected override void LoadState(Object navigationParameter, Dictionary<String, Object> pageState)
        {
            ...
            source = navigationParameter as List<DataLibrary.ExtSchedulerDTO>;
        }
        ...
        private async void itemGridView_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (item != null)
            {
                ...
                {
                    var source = await (new Lib.ServiceModel.CommonModel()).GetFIWPByProjectSchedule_ExtSch(Lib.ScheduleDataSource.selectedSchedule.ToString());

                    if (source != null && source.Count > 0)
                        this.Frame.Navigate(SubMenuDataSource.CurrentMenu, source);
                    else
                        WinAppLibrary.Utilities.Helper.SimpleMessage("There is no IWP built in the Schedule Line Item - Please create IWP first", "Warning");
                }
            }
        }
