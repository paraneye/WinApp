...
namespace Element.Reveal.TrueTask.Discipline.Schedule.BuildSIWP
{
    ...
    public sealed partial class ScheduleComponents : WinAppLibrary.Controls.LayoutAwarePage
    {
        ...
        {
            _projectid = Login.UserAccount.CurProjectID; // for demo test
            _disciplineCode = Login.UserAccount.CurDisciplineCode; //for demo test
            ...
            try
            {
                if (Login.LoginMode == WinAppLibrary.UI.LogMode.OnMode)
                {
                    await _commonsource.GetDrawingForAssignSIWPOnMode(0, _obj.systemIdList, _obj.costcodeIdList, _obj.costcodeIdList, _obj.isolinenoList, _obj.searchstringList, 
                        _obj.locationList, null, _obj.searchvalueList, _projectid, _disciplineCode, -1);
                    ...
                    await _commonsource.GetComponentProgressBySIWP(Lib.IWPDataSource.selectedSIWP, 0, _projectid, _disciplineCode);
                }
                else
                {
        ...
        private async void btnSelectedAssign_Click(object sender, RoutedEventArgs e)
        {
            ...
            List<DataLibrary.FiwpDTO> fiwplist = await (new Lib.ServiceModel.ProjectModel()).GetFiwpByID(Lib.IWPDataSource.selectedSIWP);
        ...
        private void lvUnAssignedComponent_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (e.AddedItems.Count > 0)
            {
                ...
                {
                    //endpoint선택시
                    Lib.CommonDataSource.endPoint = (e.AddedItems[0] as DataLibrary.MTODTO).IntegerVar1;
                    Lib.CommonDataSource.endDrawingID = (e.AddedItems[0] as DataLibrary.MTODTO).DrawingID;
                    btnSelectedAssign.Visibility = Visibility.Visible;
                }
                else
                {
                    //startpoint 선택시
                    Lib.CommonDataSource.startPoint = (e.AddedItems[0] as DataLibrary.MTODTO).IntegerVar1;
                    Lib.CommonDataSource.startDrawingID = (e.AddedItems[0] as DataLibrary.MTODTO).DrawingID;
                    lvUnAssignedComponent.SelectedItem = null;
