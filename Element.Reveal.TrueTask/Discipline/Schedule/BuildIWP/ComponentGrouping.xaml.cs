...
namespace Element.Reveal.TrueTask.Discipline.Schedule.BuildIWP
{
    ...
    public sealed partial class ComponentGrouping : WinAppLibrary.Controls.LayoutAwarePage
    {
        ...
        /// <summary>
        /// 가상 키보드에서 Enter 누를 시 다음 페이지 이동
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void txtSearch_KeyDown(object sender, KeyRoutedEventArgs e)
        {
            if (e.Key == Windows.System.VirtualKey.Enter) //가상키에서 enter누르면 다음 페이지 이동
            {
                MoveNextPage();
            }
        }
        ...
        private async void LoadGrouping()
        {
            ...
            try
            {
                ...
                {
                    await _commonsource.GetAvailableCollectionForScheduling(Lib.CWPDataSource.selectedCWP, 0, _projectid, _disciplineCode, Lib.IWPDataSource.selectedIWP);
