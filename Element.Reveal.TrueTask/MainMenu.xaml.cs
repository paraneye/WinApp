...
using Element.Reveal.TrueTask.Lib;
...
namespace Element.Reveal.TrueTask
{
    ...
    public sealed partial class MainMenu : WinAppLibrary.Controls.LayoutAwarePage
    {
        ...
        //{
        //    this.Frame.Navigate(typeof(Element.Reveal.TrueTask.Discipline.TimeProgress.TimeSheet), ucSelectTimeSheet.SelectedTimeSheet);
        //    GrdPopBase.Visibility = Windows.UI.Xaml.Visibility.Collapsed;
        //}
        ...
        protected override void LoadState(Object navigationParameter, Dictionary<String, Object> pageState)
        {
            ...
            GetDocumentTransitionStatusTotalCount();
            DoNext();
        }

        private async void GetDocumentTransitionStatusTotalCount()
        {
            try
            {
                totalCount = await (new Lib.ServiceModel.WorkflowModel()).GetDocumentTransitionStatusTotalCount(Login.UserAccount.PersonnelId);
            }
            catch (Exception ex)
            {
                (new WinAppLibrary.Utilities.Helper()).ExceptionHandler(ex, "MainMenu - GetDocumentTransitionStatusTotalCount");
            }
        }
        ...
        private void DoNext()
        {
            ...
            // Signoff Status에 배정 정보를 표시하기 위함.
            foreach (DataGroup group in menusource.DataSource.GetGroups("AllGroups"))
            {
                foreach (DataItem item in group.Items)
                {
                    if (item.UniqueId == "SignOffStatus")
